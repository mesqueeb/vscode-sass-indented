...
export class ColorUtilities {
   ...
   */
  static convertStringToColor(text: string): Color {
    if (text.startsWith('#')) {
...
function convertHexStringToColor(text: string) {
  ...
  return new Color(color.red / 255, color.green / 255, color.blue / 255, color.alpha / 255);
}
function convertRgbStringToColor(text: string) {
