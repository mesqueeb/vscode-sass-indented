...
const modules: { [key: string]: BasicRawCompletion[] } = {
  MATH: [
    {
      name: 'percentage()',
      body: 'percentage($1)',
    ...
    },
    {
      ...
      body: 'compatible($1, $2)',
      desc:
        'comparable($number1, $number2) - Returns whether $number1 and $number2 have compatible units. If this returns true, $number1 and $number2 can safely be added, subtracted, and compared. Otherwise, doing so will produce errors.'
    },
    {
      name: 'unit()',
      body: 'unit($1)',
      desc:
        "unit($number) - Returns a string representation of $number's units. \n\nNote: This function is intended for debugging; its output format is not guaranteed to be consistent across Sass versions or implementations."
    },
    {
      name: 'is-unitless()',
      body: 'is-unitless($1)',
...
All optional arguments must be numbers. The $red, $green, and $blue arguments must be unitless and between -255 and 255 (inclusive). The $hue argument must have either the unit deg or no unit. The $saturation and $lightness arguments must be between -100% and 100% (inclusive), and may be unitless. The $alpha argument must be unitless and between -1 and 1 (inclusive).`
    },
    {
      ...
      body: 'alpha($1)',
      desc:
        'alpha($color) - Returns the alpha channel of $color as a number between 0 and 1. As a special case, this supports the Internet Explorer syntax alpha(opacity=20), for which it returns an unquoted string.'
    },
    {
      name: 'blue()',
      body: 'blue($1)',
...
Because desaturate() is usually not the best way to make a color less saturated, it’s not included directly in the new module system. However, if you have to preserve the existing behavior, desaturate($color, $amount) can be written color.adjust($color, $saturation: -$amount).`
    ...
    },
    {
      ...
      body: 'ie-hex-str($1)',
      desc:
        'ie-hex-str($color) - Returns an unquoted string that represents $color in the #AARRGGBB format expected by Internet Explorer’s -ms-filter property.'
    },
    {
      name: 'invert()',
      body: 'invert($1)',
...
It’s an error to specify an RGB property ($red, $green, and/or $blue) at the same time as an HSL property ($saturation, and/or $lightness).`
  ...
  STRING: [
    ...
    },
    {
      ...
      body: 'index($1, $2)',
      desc:
        'index($string, $substring) - Returns the first index of $substring in $string, or null if $string doesn’t contain $substring.'
    },
    {
      name: 'insert()',
      body: 'insert($1, $2, $3)',
    ...
    },
    {
      ...
      body: 'slice($1, $2)',
      desc:
        'slice($string, $start-at, $end-at: -1) - Returns the slice of $string starting at index $start-at and ending at index $end-at (both inclusive).'
    },
    {
      name: 'to-upper-case()',
    ...
    },
    {
      ...
      body: 'unquote($1)',
      desc:
        'unquote($string) - Returns $string as an unquoted string. This can produce strings that aren’t valid CSS, so use with caution.'
    }
  ],
  LIST: [
    {
...
Use list.append() instead to add a single value to a list. Only use list.join() to combine two lists together into one`
    ...
    },
    {
      ...
      body: 'separator($1)',
      desc:
        'separator($list) - Returns the name of the separator used by $list, either space or comma. If $list doesn’t have a separator, returns space.'
    },
    {
      name: 'nth()',
      body: 'nth($1, $2)',
      desc:
        'nth($list, $n) - Returns the element of $list at index $n. If $n is negative, it counts from the end of $list. Throws an error if there is no element at index $n.'
    },
    {
      name: 'set-nth()',
      body: 'set-nth($1, $2, $3)',
      desc:
        'set-nth($list, $n, $value) - Returns a copy of $list with the element at index $n replaced with $value. If $n is negative, it counts from the end of $list. Throws an error if there is no existing element at index $n.'
    },
    {
      name: 'zip()',
      body: 'zip($1)',
  ...
  MAP: [
    {
      ...
      body: 'get($1, $2)',
      desc:
        'get($map, $key) - Returns the value in $map associated with $key. If $map doesn’t have a value associated with $key, returns null.'
    },
    {
      name: 'has-key()',
      body: 'has-key($1, $2)',
...
All keys in the returned map that also appear in $map1 have the same order as in $map1. New keys from $map2 appear at the end of the map.`
    },
    {
      ...
      body: 'remove($1, $2)',
      desc:
        'remove($map, $keys...) - Returns a copy of $map without any values associated with $keys. If a key in $keys doesn’t have an associated value in $map, it’s ignored.'
    },
    {
      name: 'values()',
      body: 'values($1)',
